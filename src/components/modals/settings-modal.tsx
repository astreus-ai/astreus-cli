...
const SETTINGS_CATEGORIES: SettingsCategory[] = [
  {
    ...
    items: [
      ...
      { key: "OPENAI_BASE_URL", label: "Base URL", value: "", placeholder: "https://api.openai.com/v1" },
      // Anthropic
      { key: "_anthropic", label: "Anthropic", value: "", isHeader: true },
      { key: "ANTHROPIC_API_KEY", label: "API Key", value: "", secret: true },
      { key: "ANTHROPIC_BASE_URL", label: "Base URL", value: "", placeholder: "https://api.anthropic.com" },
      // Gemini
      { key: "_gemini", label: "Gemini", value: "", isHeader: true },
      { key: "GEMINI_API_KEY", label: "API Key", value: "", secret: true },
      { key: "GEMINI_BASE_URL", label: "Base URL", value: "", placeholder: "https://generativelanguage.googleapis.com" },
      // Ollama
      { key: "_ollama", label: "Ollama", value: "", isHeader: true },
      { key: "OLLAMA_BASE_URL", label: "Base URL", value: "", placeholder: "http://localhost:11434" },
    ],
  },
  {
    name: "Database",
    ...
    items: [
      ...
      { key: "KNOWLEDGE_DB_URL", label: "Knowledge DB URL", value: "", placeholder: "postgresql://..." },
    ],
  },
  {
    name: "Encryption",
...
export function SettingsModal({ onClose, onSave }: SettingsModalProps) {
  ...
  const findNextEditableIndex = (items: typeof currentCategory.items, current: number, direction: 1 | -1) => {
                  ...
                  {mode === "edit" && i === itemIndex ? (
                    ...
                    <Box>
                      {item.options.map((opt, oi) => (
                        <Text
                          key={opt}
                          color={oi === optionIndex ? "yellow" : undefined}
                        >
